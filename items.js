...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        var categories = database.collection('item').aggregate([{$group: {_id: '$category', num: {$sum: 1}}}, {$sort: {_id: 1}}]).toArray(function(err, docs) {
            var sum = 0;
            for (var i in docs) {
                sum += docs[i].num;
            }
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        var pageItem = database.collection('item').find().skip(toSkip).limit(itemsPerPage).toArray(function(err, docs) {
            callback(docs);
        });
    };
    ...
    this.getNumItems = function(category, callback) {
        ...
        var numItems = database.collection('item').find().count().then(function(count) {
            callback(count);
        });
    };
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        var toSkip = page * itemsPerPage;
        var items = database.collection('item').find({
            $text: {
                $search: query
            }
        }).skip(toSkip).limit(itemsPerPage).toArray(function(err, docs) {
            callback(docs);
        });
    };
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        ...
        var numItems = database.collection('item').find({
            $text: {
                $search: query
            }
    ...
    this.getItem = function(itemId, callback) {
        ...
        var item = database.collection('item').find({_id: itemId}).limit(1).next(function(err, item) {
            callback(item);
        });
    };
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        database.collection('item').updateOne({_id: itemId}, {$push: {"reviews": reviewDoc}}, function(err, res) {
            callback(res);
        });
    };
