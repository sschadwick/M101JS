...
function CartDAO(database) {
    ...
    this.getCart = function(userId, callback) {
        ...
        */
        ...
        database.collection('cart').findOne({'userId': userId}, function(err, userCart) {
            callback(userCart);
        });
    };
    ...
    this.itemInCart = function(userId, itemId, callback) {
         ...
         */
         ...
         database.collection('cart').findOne({"items._id": itemId, "userId": userId}, function(err, res) {
            if (res) { callback(res); }
            callback(null);
         });
    };
    ...
    this.updateQuantity = function(userId, itemId, quantity, callback) {
        ...
        */
        ...
        var cmd;
        if (quantity === 0) {
            cmd = {"$pull": {"items": {"_id": itemId}}};
        } else {
            cmd = {"$set": {"items.$.quantity": quantity}};
        }
